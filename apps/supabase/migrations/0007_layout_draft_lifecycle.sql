-- 0007_layout_draft_lifecycle.sql
...
begin
  ...
  loop
    insert into public.racks (
      ...
      new_version_uuid,
      rack_row.display_code,
      rack_row.kind,
      rack_row.axis,
      rack_row.x,
      rack_row.y,
      rack_row.total_length,
      rack_row.depth,
      rack_row.rotation_deg,
      'draft'
    )
    ...
    for old_face_id in
      ...
      where rf.rack_id = (select id from public.racks where layout_version_id = published_version.id and display_code = rack_row.display_code limit 1)
      order by rf.side
    loop
      ...
      for section_row in
        ...
        where rs.rack_face_id = old_face_id
        order by rs.ordinal
      loop
        old_section_id := section_row.id;

        insert into public.rack_sections (
          rack_face_id,
          ordinal,
          length
        ) values (
        ...
        section_id_map := jsonb_set(section_id_map, array[old_section_id::text], to_jsonb(new_section_id::text));

        insert into public.rack_levels (
          rack_section_id,
          ordinal,
          slot_count
        )
...
create or replace function public.save_layout_draft(layout_payload jsonb)
returns uuid
language plpgsql
as $$
declare
