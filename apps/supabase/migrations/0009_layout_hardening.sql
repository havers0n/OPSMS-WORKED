-- 0009_layout_hardening.sql

create or replace function public.build_cell_code(
  rack_uuid uuid,
  face_side text,
...
alter table public.cells
  add column if not exists cell_code text;

update public.cells c
set cell_code = public.build_cell_code(c.rack_id, rf.side, rs.ordinal, rl.ordinal, c.slot_no)
from public.rack_faces rf,
     public.rack_sections rs,
     public.rack_levels rl
where rf.id = c.rack_face_id
  and rs.id = c.rack_section_id
  and rl.id = c.rack_level_id
  and c.cell_code is null;

alter table public.cells
  alter column cell_code set not null;

alter table public.cells
  drop constraint if exists cells_layout_cell_code_unique;
...
create index if not exists operation_events_layout_version_idx
  on public.operation_events (layout_version_id, created_at desc);

create index if not exists operation_events_floor_idx
  on public.operation_events (floor_id, created_at desc);
...
after insert or update of layout_version_id, rack_id, rack_face_id, rack_section_id, rack_level_id
on public.cells
deferrable initially immediate
for each row
execute function public.validate_cells_tree_consistency();

create or replace function public.validate_rack_face_mirror_consistency()
returns trigger
language plpgsql
as $$
declare
...
after insert or update of rack_id, side, is_mirrored, mirror_source_face_id
  ...
  loop
    ...
    loop
      ...
      if not exists (
        ...
        issues := issues || jsonb_build_object('code', 'rack_face.section_length_mismatch', 'severity', 'error', 'entityId', face_record.id, 'message', format('Face %s section length sum (%s) does not match rack total length (%s).', face_record.side, section_length_sum, rack_record.total_length));
        issue_count := issue_count + 1;
      end if;
...
begin
  ...
  loop
    ...
    for old_face_id in
      ...
      order by rf.side
    loop
      insert into public.rack_faces (rack_id, side, enabled, anchor, slot_numbering_direction, is_mirrored, mirror_source_face_id)
      select rack_row.id, rf.side, rf.enabled, rf.anchor, rf.slot_numbering_direction, rf.is_mirrored, null
      from public.rack_faces rf
...
drop function if exists public.save_layout_draft(jsonb);
...
comment on function public.save_layout_draft(jsonb, uuid)
  is 'Transitional editor-persistence RPC only. Do not attach new downstream business logic or dependencies to this destructive rewrite path.';
