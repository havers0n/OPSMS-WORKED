-- 0080_layout_zones.sql
--
-- Narrow first slice for floor-level semantic zones:
-- rectangular persisted layout entities identified primarily by user-entered name,
-- with optional lightweight category presets and no runtime policy behavior.
...
drop policy if exists layout_zones_select_scoped on public.layout_zones;
create policy layout_zones_select_scoped
on public.layout_zones
for select
to authenticated
using (public.can_access_layout_version(layout_version_id));

drop policy if exists layout_zones_insert_scoped on public.layout_zones;
create policy layout_zones_insert_scoped
on public.layout_zones
for insert
to authenticated
with check (public.can_manage_layout_version(layout_version_id));

drop policy if exists layout_zones_update_scoped on public.layout_zones;
create policy layout_zones_update_scoped
on public.layout_zones
for update
to authenticated
...
drop policy if exists layout_zones_delete_scoped on public.layout_zones;
create policy layout_zones_delete_scoped
on public.layout_zones
for delete
to authenticated
using (public.can_manage_layout_version(layout_version_id));
