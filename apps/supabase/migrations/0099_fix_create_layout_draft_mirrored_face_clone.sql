-- Migration 0099: avoid transient invalid mirrored faces while cloning a draft.
--
-- create_layout_draft clones published rack_faces in two phases because mirror
-- source ids must be remapped from published face ids to draft face ids. The
-- mirrored relationship therefore must not be present on the initial INSERT:
-- deferred constraint triggers keep the INSERT row image and can validate it
-- before considering the later UPDATE that fills mirror_source_face_id.
...
comment on function public.create_layout_draft(uuid, uuid) is
  'Creates a draft layout version, cloning published layout data with mirror source ids remapped after initial independent face insertion.';
