-- 0113_disable_empty_removed_locations_on_publish.sql
--
-- Phase 2: after the destructive location guard passes, disable removed
-- rack-slot locations that have no blocking operational references.
...
begin
  ...
  -- blocking references. Disable them before archiving the old layout so the
  -- existing geometry_slot_id still points at a trigger-valid published cell.
  with old_published_codes as (
    select distinct coalesce(c.address, c.cell_code) as code
    from public.layout_versions lv
    join public.cells c on c.layout_version_id = lv.id
