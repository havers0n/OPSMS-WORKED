-- 0116_rack_layout_lock.sql
--
-- Adds a persisted per-rack layout lock. Locked racks remain selectable, but
-- editor clients must not change their geometry, placement, or structure.
...
begin
  ...
  insert into public.racks (
    ...
    is_locked, x, y, total_length, depth, rotation_deg, state
  )
  select
    (r ->> 'id')::uuid,
    layout_version_uuid,
    ...
    r ->> 'kind',
    r ->> 'axis',
    coalesce((r ->> 'isLocked')::boolean, false),
    (r ->> 'x')::numeric,
    (r ->> 'y')::numeric,
...
begin
  ...
  insert into public.racks (id, layout_version_id, display_code, kind, axis, is_locked, x, y, total_length, depth, rotation_deg, state)
  select m.new_id, new_version_uuid, r.display_code, r.kind, r.axis, r.is_locked, r.x, r.y, r.total_length, r.depth, r.rotation_deg, 'draft'
