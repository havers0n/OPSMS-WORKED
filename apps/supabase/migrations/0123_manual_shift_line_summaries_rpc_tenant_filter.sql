-- Migration: 0123_manual_shift_line_summaries_rpc_tenant_filter.sql
-- Description: Add explicit p_tenant_id param to manual_shift_list_line_summaries for defense-in-depth tenant isolation.
-- The function is SECURITY INVOKER so RLS still applies; this makes tenant ownership
-- explicit at the query level regardless of caller context.

-- Drop the old single-parameter overload so the new signature takes over cleanly.
drop function if exists public.manual_shift_list_line_summaries(uuid);
