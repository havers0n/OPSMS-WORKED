-- Description: PR B - preserve order-level source zone on manual_shift_orders.
-- Adds nullable source_zone, backfills from item zones when unambiguous,
-- and persists source_zone during monthly import apply.
--
-- Backfill policy:
--   0 distinct non-null item zones -> source_zone stays null
--   1 distinct non-null item zone  -> source_zone = that zone
--   >1 distinct non-null item zones -> source_zone stays null
--
-- Diagnostic query for mixed-zone orders:
--   select o.id, o.order_number,
--          array_agg(distinct nullif(btrim(i.zone), '') order by nullif(btrim(i.zone), '')) as item_zones
--   from public.manual_shift_orders o
--   left join public.manual_shift_order_items i on i.order_id = o.id
--   group by o.id, o.order_number
--   having count(distinct nullif(btrim(i.zone), '')) > 1;

alter table public.manual_shift_orders
  add column if not exists source_zone text null;

create index if not exists manual_shift_orders_shift_line_source_zone_point_idx
  on public.manual_shift_orders (shift_id, line_id, source_zone, point_name);
  ...
  loop
    ...
    loop
      ...
      values (
        ...
        'unknown',
        'queued',
        coalesce(v_order."rawRouteLine",
          CASE
            WHEN v_order."workBucketName" IS NOT NULL
            THEN v_line."lineName" || '/' || v_order."workBucketName"
            ELSE v_line."lineName"
          END
        ),
        coalesce(v_order."routeBase", v_line."lineName"),
        v_order."workBucketName",
        v_order."workBucketType",
        nullif(btrim(v_order."sourceZone"), '')
      )
