-- 0071_fix_publish_layout_version_exception_handler.sql
--
-- Bug: When publish_layout_version is called with a non-existent layout_version_uuid,
-- the exception handler tries to insert into operation_events with a layout_version_id
-- that doesn't exist, violating the FK constraint.
--
-- Fix: Guard write_layout_event with `floor_uuid is not null` — floor_uuid is only
-- populated after the version lookup succeeds, so null means the version doesn't exist.
