...
begin
  ...
  result := public.apply_demand_datasheet_import(
    ...
    '{"rowsCount":3,"rawRowsCount":3,"warningRowsCount":1,"errorRowsCount":1,"specialFlowRowsCount":0,"distributionAreasCount":1,"distinctOrdersCount":3,"distinctSkuCount":3}',
    jsonb_build_array(
  ...
  if row_count <> 3 then raise exception 'Expected all 3 raw rows, got %',row_count; end if;
  select count(*) into row_count from public.demand_backlog_items where tenant_id=tenant_a;
  if row_count <> 2 then raise exception 'Expected two identity-valid backlog items, got %',row_count; end if;
  ...
  if row_count <> 2 then raise exception 'Expected two source links with quantity metadata, got %',row_count; end if;

  dry_result := public.repair_demand_backlog(tenant_a,batch_a,true);
  if (dry_result->>'rawRowsScanned')::int <> 3 or (dry_result->>'sourceLinksUnchanged')::int <> 2 then
  ...
  if row_count <> 2 then raise exception 'Dry-run changed persistent source links'; end if;

  result := public.repair_demand_backlog(tenant_a,batch_a,false);
  if (result->>'sourceLinksCreated')::int <> 0 or (result->>'sourceLinksUnchanged')::int <> 2 then
