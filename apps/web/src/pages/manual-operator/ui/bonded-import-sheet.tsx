...
interface BondedImportSheetProps {
  shiftId?: string | null;
...
export function BondedImportSheet({ shiftId, selectedDate, onClose }: BondedImportSheetProps) {
    ...
    <div className="fixed inset-0 z-50 flex items-end justify-center" dir="rtl">
      ...
      <div className="relative w-full max-w-[430px] h-[100dvh] bg-white flex flex-col">
        <div className="sticky top-0 bg-white border-b border-gray-200 p-4 flex items-center justify-between">
          <h2 className="font-bold text-lg text-gray-900">טעינת קובץ בונדד</h2>
          <button
            onClick={onClose}
            disabled={isBlocking}
              ...
              <div className="rounded-xl border border-gray-200 p-4 text-sm space-y-1">
                ...
                {pivotFound && (
                  <div className="rounded-lg bg-amber-50 border border-amber-200 text-amber-800 text-xs p-2 mt-2">
                    PIVOT! обнаружен и проигнорирован
                  </div>
                )}
                {draft.diagnostics.warnings.length > 0 && (
                  <div className="mt-2 space-y-1">
                    {draft.diagnostics.warnings.map((w, i) => (
              ...
              <div className="space-y-2 max-h-48 overflow-y-auto">
                {snapshots.map((snap) => (
                  <div key={snap.id}>
                    <button
                      type="button"
                      onClick={() => setViewingSnapshotId(viewingSnapshotId === snap.id ? null : snap.id)}
                      className="w-full text-right p-2 rounded-lg hover:bg-gray-50 text-sm space-y-0.5"
                    >
                      <p><span className="font-medium">תאריך עבודה:</span> {snap.planningDate}</p>
                      <p><span className="font-medium">קובץ:</span> {snap.fileName || '—'}</p>
                      <p><span className="font-medium">הועלה:</span> {formatDateTime(snap.importedAt)}</p>
                      <p><span className="font-medium">שורות:</span> {snap.rowCount}</p>
                      <p><span className="font-medium">סטטוס:</span> {snap.status}</p>
                      {snap.diagnostics.warnings.length > 0 && (
                        <p className="text-xs text-amber-600">{snap.diagnostics.warnings.length} אזהרות</p>
                      )}
                    </button>
                  </div>
                ))}
