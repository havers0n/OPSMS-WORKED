...
function WaveProgress({ readyOrders, totalOrders }: { readyOrders: number; totalOrders: number }) {
  const t = useT();
  if (totalOrders === 0) return <span className="text-xs text-slate-400">{t('operations.page.empty.progress')}</span>;
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
      <div className="w-full max-w-sm rounded-2xl bg-white p-6 shadow-xl">
        <div className="text-lg font-semibold text-slate-900">{t('operations.page.action.newWave')}</div>
        <p className="mt-1 text-sm text-slate-500">{t('operations.page.modal.newWave.description')}</p>
        <form className="mt-4 space-y-3" onSubmit={(e) => { e.preventDefault(); if (!name.trim()) return; createWave.mutate({ name: name.trim() }, { onSuccess: (w) => onCreated(w.id) }); }}>
          <input value={name} onChange={(e) => setName(e.target.value)} placeholder={t('operations.page.placeholder.waveName')} dir="auto" className="h-10 w-full rounded-xl border border-slate-300 px-3 text-sm outline-none focus:border-cyan-500" autoFocus />
          <div className="flex gap-2">
            <button type="button" onClick={onClose} className="flex-1 rounded-xl border border-slate-200 py-2 text-sm text-slate-600 hover:bg-slate-50">{t('storage.action.cancel')}</button>
            <button type="submit" disabled={createWave.isPending || !name.trim()} className="flex-1 rounded-xl bg-cyan-600 py-2 text-sm font-medium text-white hover:bg-cyan-500 disabled:opacity-50">
              {createWave.isPending ? t('warehouse.action.creating') : t('operations.page.action.createAndOpen')}
            </button>
          </div>
        </form>
      </div>
...
function CreateOrderModal({ onClose }: { onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
      <div className="w-full max-w-sm rounded-2xl bg-white p-6 shadow-xl">
        <div className="text-lg font-semibold text-slate-900">{t('operations.page.action.quickOrder')}</div>
        <p className="mt-1 text-sm text-slate-500">{t('operations.page.modal.quickOrder.description')}</p>
        <form className="mt-4 space-y-3" onSubmit={(e) => { e.preventDefault(); if (!externalNumber.trim()) return; createOrder.mutate({ externalNumber: externalNumber.trim() }, { onSuccess: onClose }); }}>
          <input value={externalNumber} onChange={(e) => setExternalNumber(e.target.value)} placeholder={t('operations.page.placeholder.orderNumber')} dir="ltr" className="h-10 w-full rounded-xl border border-slate-300 px-3 text-sm outline-none focus:border-cyan-500" autoFocus />
          <div className="flex gap-2">
            <button type="button" onClick={onClose} className="flex-1 rounded-xl border border-slate-200 py-2 text-sm text-slate-600 hover:bg-slate-50">{t('storage.action.cancel')}</button>
            <button type="submit" disabled={createOrder.isPending || !externalNumber.trim()} className="flex-1 rounded-xl bg-slate-800 py-2 text-sm font-medium text-white hover:bg-slate-700 disabled:opacity-50">
              {createOrder.isPending ? t('warehouse.action.creating') : t('operations.page.action.createOrder')}
            </button>
          </div>
        </form>
      </div>
      ...
      <div className="flex flex-1 flex-col overflow-hidden">
          ...
          <div className="flex items-center justify-between gap-4">
            <div className="flex items-center gap-2">
              ...
              >
                ...
                {t('operations.page.action.newWave')}
              </button>
              <button
                type="button"
              ...
              >
                ...
                {t('operations.page.action.quickOrder')}
              </button>
            </div>
          </div>
        </div>
        ...
        <div className="border-b border-slate-200 bg-white px-6">
          <div className="flex gap-1">
            {(['waves', 'orders'] as const).map((tab) => (
              ...
              >
                {tab === 'waves' ? t('operations.page.tab.waves') : t('operations.page.tab.standaloneOrders')}
                <span
        ...
        <div className="flex-1 overflow-auto p-6">
          {activeTab === 'waves' && (
            <section>
              <div className="mb-3 flex items-center justify-between">
                <button type="button" aria-label={t('operations.page.action.refreshWaves')} onClick={() => void refetchWaves()} className="flex h-7 w-7 items-center justify-center rounded-lg text-slate-400 hover:bg-slate-100 hover:text-slate-600">
              ...
              {wavesLoading ? (
                ...
                <div className="flex h-24 items-center justify-center rounded-2xl border border-dashed border-slate-200 text-sm text-slate-400">
                  {t('operations.page.empty.noWaves')}
                </div>
              ) : (
                ...
                <thead className="bg-slate-50 text-start text-xs uppercase tracking-wide text-slate-500">
                  <tr>
                    <th className="px-5 py-3">{t('operations.page.table.wave')}</th>
                    <th className="px-4 py-3">{t('operations.page.table.status')}</th>
                    <th className="px-4 py-3">{t('operations.page.table.progress')}</th>
                    <th className="px-4 py-3 text-end">{t('operations.page.table.actions')}</th>
                  </tr>
                  </thead>
                            ...
                            <div className="flex items-center gap-2 mt-1">
                              {wave.blockingOrderCount > 0 ? (
                                <div className="flex items-center gap-1 text-xs text-amber-700">
                                  ...
                                  <span>{t(wave.blockingOrderCount === 1 ? 'operations.page.wave.blocker' : 'operations.page.wave.blockers', { count: wave.blockingOrderCount })}</span>
                                </div>
                              ) : wave.totalOrders > 0 ? (
                                <div className="text-xs text-emerald-700">{t('operations.page.wave.allReady')}</div>
                              ) : (
                                <div className="text-xs text-slate-400">{t('operations.page.wave.noOrders')}</div>
                              )}
                            </div>
                          </td>
          ...
          {activeTab === 'orders' && (
            <section>
              <div className="mb-3 flex items-center justify-between">
                <button type="button" aria-label={t('operations.page.action.refreshOrders')} onClick={() => void refetchOrders()} className="flex h-7 w-7 items-center justify-center rounded-lg text-slate-400 hover:bg-slate-100 hover:text-slate-600">
              ...
              {ordersLoading ? (
                ...
                <div className="flex h-24 items-center justify-center rounded-2xl border border-dashed border-slate-200 text-sm text-slate-400">
                  {t('operations.page.empty.noStandaloneOrders')}
                </div>
              ) : (
                <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white">
                  <table className="min-w-full divide-y divide-slate-100 text-sm">
                  <thead className="bg-slate-50 text-start text-xs uppercase tracking-wide text-slate-500">
                    <tr>
                      <th className="px-5 py-3">{t('operations.page.table.order')}</th>
                      <th className="px-4 py-3">{t('operations.page.table.status')}</th>
                      <th className="px-4 py-3">{t('operations.page.table.lines')}</th>
                      <th className="px-4 py-3">{t('operations.page.table.progress')}</th>
                      <th className="px-4 py-3 text-end">{t('operations.page.table.actions')}</th>
                    </tr>
                  </thead>
