...
export function WarehouseSetupPage() {
  ...
  const draftQuery = useActiveLayoutDraft(activeFloorId);
  ...
  const setupState: WarehouseSetupState = useMemo(() => {
    if (sitesQuery.isLoading || (activeSiteId ? floorsQuery.isLoading : false) || (activeFloorId ? draftQuery.isLoading : false)) {
      return 'draft_loading';
    }

    if (sitesQuery.isError || floorsQuery.isError || draftQuery.isError) {
      return 'error';
    }
    ...
    return 'floor_selection_required';
  }, [activeFloorId, activeSiteId, draftQuery.data, draftQuery.isError, draftQuery.isLoading, floorsQuery.data, floorsQuery.isError, floorsQuery.isLoading, sitesQuery.data, sitesQuery.isError, sitesQuery.isLoading]);
    ...
    return (
      <div className="flex h-full w-full items-center justify-center">
        <div className="w-full max-w-lg rounded-[22px] border border-[var(--border-muted)] bg-[var(--surface-primary)] p-8 text-center shadow-[var(--shadow-soft)]">
          ...
          <div className="mt-2 text-sm text-[var(--text-muted)]">Resolving current site, floor, and draft state from the live layout lifecycle.</div>
        </div>
      </div>
    );
  }
  ...
  if (setupState === 'floor_selection_required') {
    return <SiteFloorSetupState hasDraft={Boolean(draftQuery.data)} />;
  }
