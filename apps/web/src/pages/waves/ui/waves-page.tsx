...
import { getWaveStatusColor, getWaveStatusLabel, getWavePrimaryAction, getWaveSecondaryAction } from '@/entities/wave/lib/wave-actions';
...
function WaveDrawer({ waveId, onClose }: { waveId: string; onClose: () => void }) {
  ...
  const primaryAction = getWavePrimaryAction(wave);
        ...
        <div className="flex flex-wrap gap-2">
          {secondaryAction?.target && (
            <button
              ...
              onClick={() =>
                transition.mutate({ waveId: wave.id, status: secondaryAction.target as WaveStatus })
              }
