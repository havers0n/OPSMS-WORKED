...
const items: NavItem[] = [
  { label: 'Warehouse', icon: LayoutGrid, to: routes.warehouse },
  { label: 'Products', icon: Package, disabled: true },
  { label: 'Operations', icon: Activity, to: routes.operations },
          ...
          if ('disabled' in item) {
            return (
              <button
                ...
                disabled
                className={[
                  'flex cursor-not-allowed items-center rounded-xl border border-transparent py-2.5 text-left text-sm text-slate-500 opacity-80',
                  isCollapsed ? 'justify-center px-2' : 'gap-3 px-2'
                ].join(' ')}
              >
                <span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-slate-900 text-slate-600">
                ...
                {!isCollapsed && (
                  <span className="min-w-0 flex-1">
                    ...
                    <span className="mt-0.5 block truncate text-xs text-slate-600">Soon</span>
                  </span>
                )}
              </button>
            );
