...
| Cells are generated by the system, not authored manually in normal flow | domain + DB | `generateLayoutCells`, `validate_layout_version`, `publish_layout_version` | domain unit + SQL + E2E | critical; partial: strongly enforced on app publish flow, but not sealed against privileged direct SQL writes to `cells` |
